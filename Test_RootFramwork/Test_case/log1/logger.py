...
# 这个是日志保存本地的路径
log_path = "C:\\Users\\john\\Desktop\\test2018\\Test_RootFramwork\\Test_case\\log1"
class log:
    def __init__(self):
        # 文件的命名
...
# log_path="C:\\Users\\john\\Desktop\\test2018\\Test_RootFramwork\\Test_case\\logging"
# class log:
#     def __init__(self):
#         #文件命名
#         self.logname=os.path.join(log_path,'%s.log'%time.strftime('%Y_%m_%d_%H_%M_%S'))
...
#         self.logger.setLevel(logging.DEBUG)
#         #日志输入格式
#         self.formatter=logging.Formatter('[%(asctime)s]-%(filename)s[line:%(lineno)d]-fuc:%(funcName)s-%(levename)s:%(message)s')
#     def __console(self,level,message):
