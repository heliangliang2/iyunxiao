...
if __name__=='__main__':
    test_dir='C:\\Users\\Administrator\\Desktop\\iyunxiao\\iyunxiao\\Test_iyunxiao\\Test_case\\'
    #C:\Users\john\Desktop\test2018\Test_RootFramwork\Test_case
    test_report='C:\\Users\\Administrator\\Desktop\\iyunxiao\\iyunxiao\\Test_iyunxiao\\Report\\'
    discover=unittest.defaultTestLoader.discover(test_dir,pattern='test_*.py')
    now =time.strftime("%Y-%m-%d_%H_%M_%S")
    filename=test_report+'\\'+now+'result.html'
    fp=open(filename,'wb')
    runner=HTMLTestRunner(stream=fp,title='好分数运营管理中心测试报告',description='用例执行情况')
    runner.run(discover)
    fp.close()
